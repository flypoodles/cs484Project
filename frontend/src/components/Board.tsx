...
export default function Board({
  ...
  const [firstPiece, setFirstPiece] = useState<PieceType>({
    piece: "none",
    row: -1,
    col: -1,
  });
  const [finalPiece, setFinalPiece] = useState<PieceType>({
    piece: "none",
    row: -1,
    col: -1,
  });
  ...
  useEffect(() => {
    if (finalPiece.piece !== "none") {
      ...
      socket.emit(
        "move",
        [firstPiece.row, firstPiece.col],
        [finalPiece.row, finalPiece.col],
        firstPiece.piece,
        boardFen
      );
      setYourTurn(false);
      ...
      {board.flatMap((row, i) =>
        row.map((piece, j) => (
          <Piece
            key={`${i}-${j}`}
            isVisible={
              comparePiece(firstPiece, { piece: piece, row: i, col: j })
                ? true
                : false
            }
            handleClickPiece={() =>
              onClickPiece({ piece: piece, row: i, col: j })
            }
